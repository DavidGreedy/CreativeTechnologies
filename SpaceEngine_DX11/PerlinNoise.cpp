...
float PerlinNoise::Noise4(float x, float y, float z, float w)
{
...
#pragma region Periodic Noise
float PerlinNoise::Periodic1D(float x, int px)
{
	int ix0, ix1;
	float fx0, fx1;
	float s, n0, n1;
...
float PerlinNoise::Periodic2D(float x, float y, int px, int py)
{
	int ix0, iy0, ix1, iy1;
	float fx0, fy0, fx1, fy1;
	float s, t, nx0, nx1, n0, n1;
...
float PerlinNoise::Periodic3D(float x, float y, float z, int px, int py, int pz)
{
	int ix0, iy0, ix1, iy1, iz0, iz1;
	float fx0, fy0, fz0, fx1, fy1, fz1;
	float s, t, r;
...
float PerlinNoise::Periodic4D(float x, float y, float z, float w, int px, int py, int pz, int pw)
{
	int ix0, iy0, iz0, iw0, ix1, iy1, iz1, iw1;
	float fx0, fy0, fz0, fw0, fx1, fy1, fz1, fw1;
	float s, t, r, q;
...
float PerlinNoise::FractalPeriodic1D(float x, int px, int octaves, float persistence)
{

	return 0.0f;
}

float PerlinNoise::FractalPeriodic2D(float x, float y, int px, int py, int octaves, float persistence)
{
	float fNoise = 0.0f;
	for (size_t i = 0; i < octaves; i++)
	{
		fNoise += Periodic2D(x, y, px, py) * persistence;
		persistence *= persistence;
	}
	return fNoise;
}

float PerlinNoise::FractalPeriodic3D(float x, float y, float z, int px, int py, int pz, int octaves, float persistence)
{
	float fNoise = 0.0f;
	float freq = 1.0f;
	...
	for (size_t i = 0; i < octaves; i++)
	{
		fNoise += Periodic3D(x * freq, y * freq, z * freq, px, py, pz) * persistence;
		persistence *= persistence;
		freq *= 2.0f;
	}
...
float PerlinNoise::FractalPeriodic3D(XMFLOAT3 position, XMFLOAT3 period, int octaves, float persistence)
{
	float fNoise = 0.0f;
	float freq = 0.005f;
	...
	{
		fNoise += Periodic3D(position.x * freq, position.y * freq, position.z * freq, period.x, period.y, period.z) * persistence;
		persistence *= persistence;
		freq *= 5.0f;
	}
	return fNoise;
...
float PerlinNoise::Grad1(int hash, float x)
{
	int h = hash & 15;
...
float PerlinNoise::Grad4(int hash, float x, float y, float z, float t)
{
